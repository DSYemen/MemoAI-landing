...
const Navbar: FC = () => {
  ...
  const updateTexts = (currentDirection: string) => {
    const lang = currentDirection === 'rtl' ? 'ar' : 'en';
    setTexts({
      memoAI: lang === 'ar' ? 'ميمو AI' : 'MemoAI',
      features: lang === 'ar' ? 'الميزات' : 'Features',
      aiPreview: lang === 'ar' ? 'معاينة AI' : 'AI Preview',
  ...
  useEffect(() => {
    ...
    const handleDirectionChange = (event: Event) => {
      ...
      updateTexts(newDirection);
    };
    window.addEventListener('directionChangedInternal', handleDirectionChange);

    return () => {
      window.removeEventListener('directionChangedInternal', handleDirectionChange);
    };
  ...
  const toggleDirection = () => {
    ...
    setDirection(newDirection); // Update state first
    localStorage.setItem('direction', newDirection);
    document.documentElement.setAttribute('dir', newDirection);
    updateTexts(newDirection); // Update texts based on the new direction
    window.dispatchEvent(new CustomEvent('directionChanged', { detail: { direction: newDirection } }));
     // Dispatch a slightly different event for internal navbar updates if needed, or rely on state
    window.dispatchEvent(new CustomEvent('directionChangedInternal', { detail: { direction: newDirection } }));
  };
